...
public enum WeaponCycleTypeOld
{
    Instant,
    Pump
}
...
public class WeaponControllerOld : MonoBehaviour
{
    ...
    [SerializeField] private WeaponCycleTypeOld cycleType = WeaponCycleTypeOld.Instant;
    ...
    private void HandleShellEjection()
    {
        switch (cycleType)
        {
            case WeaponCycleTypeOld.Instant:
                ShootShell();
                break;

            case WeaponCycleTypeOld.Pump:
    ...
    {
        if (cycleType != WeaponCycleTypeOld.Pump)
            return;
